...
// 1. Hitung total mahasiswa bimbingan
$stmt = $pdo->prepare("
    SELECT COUNT(*) as total 
    FROM penempatan 
    WHERE dpl_id = ? AND status = 'aktif'
");
$stmt->execute([$user_id]);
$total_mahasiswa = $stmt->fetch()['total'];

// 2. Hitung total lokasi KKN yang ada
$stmt = $pdo->query("SELECT COUNT(*) as total FROM lokasi");
$total_lokasi = $stmt->fetch()['total'];

// 3. Hitung kegiatan yang sedang berjalan
$stmt = $pdo->query("
    SELECT COUNT(*) as total 
    FROM kegiatan 
    WHERE status = 'berjalan'
");
$kegiatan_berjalan = $stmt->fetch()['total'];

// 4. Hitung laporan yang pending verifikasi
$stmt = $pdo->query("
    SELECT COUNT(*) as total 
    FROM laporan 
...
// 5. Ambil laporan pending untuk ditampilkan di tabel
$stmt = $pdo->query("
    SELECT 
        l.id as laporan_id,
        l.tanggal_laporan,
...
<head>
    ...
    <title>Dashboard DPL - KKN Tracking</title>
    <style>
        :root {
            --primary: #6366f1;
            --primary-dark: #4f46e5;
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; }
        ...
        .stat-card.lokasi { border-left-color: var(--accent); }
        .stat-card.kegiatan { border-left-color: var(--warning); }
...
<aside class="sidebar" id="sidebar">
    ...
    <main class="main-content">
        <!-- Top Bar -->
        <div class="top-bar">
            <div class="page-title">Dashboard DPL</div>
    ...
    <div class="welcome-card">
        <div class="welcome-content">
            ...
            <p>
                Kelola dan pantau aktivitas mahasiswa KKN dengan lebih mudah dan terstruktur
            </p>
        </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><strong><?= htmlspecialchars($laporan['nama_mahasiswa']) ?></strong></td>
                                <td><?= htmlspecialchars($laporan['judul_kegiatan']) ?></td>
                                <td><?= ucfirst($laporan['jenis_laporan']) ?></td>
                                <td><span class="badge badge-pending">Pending</span></td>
                                <td>
                                    <a href="verifikasi_laporan.php" class="btn btn-primary">Verifikasi</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
    ...
    <p>
        <strong>Semua laporan sudah diverifikasi</strong>
    </p>

    <p style="font-size:13px; color:#94a3b8;">
        Tidak ada laporan yang menunggu proses verifikasi saat ini
    </p>
</div>
                <?php endif; ?>
            </div>
